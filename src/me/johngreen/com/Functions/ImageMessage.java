...
public class ImageMessage {
    ...
    public ImageMessage appendCenteredText(String... text) {
    	...
    	String[] message2 = stringBuilder(message);
        for (int y = 0; y < lines.length; y++) {
            if ((text.length-1+message2.length) > y) {
                int len = ChatPaginator.AVERAGE_CHAT_PAGE_WIDTH - lines[y].length();
                if(text.length-1>y){
                	lines[y] = lines[y] + center(text[y], len);
                }else if(text.length>y-3){
                	lines[y] = lines[y] + center(message2[y-3], len);
                }
